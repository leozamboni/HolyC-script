...
const HOLYSCRIPTTAGS = document.getElementsByTagName('holy-script')

const STDOUTERRTAG = document.createElement("p");
STDOUTERRTAG.setAttribute('id','stdout/stderr');
STDOUTERRTAG.style.cssText = 'border: 4mm ridge rgba(43, 106, 205, 0.61);'

let STDINSTR = ''
for (const hs of HOLYSCRIPTTAGS) {
  STDINSTR += hs.textContent
}

HOLYSCRIPTTAGS[HOLYSCRIPTTAGS.length - 1].append(STDOUTERRTAG)

holy_script(STDINSTR)
